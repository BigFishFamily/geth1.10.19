...
func (s *PublicBlockChainAPI) GetCodeText(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash) (string, error) {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return "", err
	}
	code := state.GetCodeText(address)
	return code, state.Error()
}
...
// block number. The rpc.LatestBlockNumber and rpc.PendingBlockNumber meta block
// numbers are also allowed.
func (s *PublicBlockChainAPI) GetStorageAt222(ctx context.Context, address common.Address, key string, blockNrOrHash rpc.BlockNumberOrHash) (hexutil.Bytes, error) {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return nil, err
	}
	fmt.Println("INFO [internal-ethapi-api.go-GetStorageAt222,input param:]-[ address: ", address.String(), "; key: ", key, "; blockNrOrHash: ", blockNrOrHash, "]")
	res := state.GetState222(address, common.HexToHash(key))
	return res[:], state.Error()
}

func (s *PublicBlockChainAPI) GetStorageByAddr(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash) ([]map[string]common.Hash, error) {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return nil, err
	}
	return state.GetStateByAddr(address)
}

//return account state(balance, nonce code hash and state root) according to address and blocknumber
func (s *PublicBlockChainAPI) GetAccountInfo(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash) (*types.StateAccount, error) {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return nil, err
	}
	return state.GetAccountInfo(address)
}

//return account state(balance, nonce code hash and state root) according to address and blocknumber
func (s *PublicBlockChainAPI) GetStorageByAddrKeyAndNumber(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash, startKey common.Hash, number int) ([]map[string]common.Hash, error) {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return nil, err
	}
	return state.GetStorageByAddrKeyAndNumber(address, startKey, number)
}

//return account state(balance, nonce code hash and state root) according to address and blocknumber
func (s *PublicBlockChainAPI) GetBlockAccountInfo222(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (map[common.Hash]common.AccInfo, error) {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return nil, err
	}
	return state.GetBlockAccountInfo222()
}
...
func (s *PublicBlockChainAPI) GetBlockReceipts(ctx context.Context, hash common.Hash) (types.Receipts, error) {
...
//wxx
func (s *PublicTransactionPoolAPI) SendPrivateNoP2pRawTransaction(ctx context.Context, encodedTx hexutil.Bytes) (common.Hash, error) {
	tx := new(types.Transaction)
...
func (api *PrivateDebugAPI) ChaindbCompact() error {
	//fmt.Println("[INFOCCC ", time.Now(), "] internal->ethapi->api.go->ChaindbCompact")
