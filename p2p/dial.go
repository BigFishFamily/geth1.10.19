...
func (d *dialScheduler) startStaticDials(n int) (started int) {
	started = 0
	//trusted node first
	for idx := 0; idx  <len(d.staticPool) ; idx++ {
		//idx := d.rand.Intn(len(d.staticPool))
		task := d.staticPool[idx]
		if task != nil && task.needTrustedConnResolve() {
			d.startDial(task)
	...
	for ; started < n && len(d.staticPool) > 0; started++ {
		idx := d.rand.Intn(len(d.staticPool))
		task := d.staticPool[idx]
		d.startDial(task)
		d.removeFromStaticPool(idx)
...
func (t *dialTask) needTrustedConnResolve() bool {
	return t.flags&trustedConn != 0 && t.dest.IP() == nil
}
