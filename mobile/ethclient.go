...
func (ec *EthereumClient) GetStorageAt222(ctx *Context, account *Address, key *Hash, number int64) (storage []byte, _ error) {
	if number < 0 {
		return ec.client.StorageAt222(ctx.context, account.address, key.hash, nil)
	}
	return ec.client.StorageAt222(ctx.context, account.address, key.hash, big.NewInt(number))
}

func (ec *EthereumClient) GetStorageByAddr(ctx *Context, account *Address, number int64) (allStorage []map[string]common.Hash, _ error) {
	if number < 0 {
		return ec.client.StorageByAddr(ctx.context, account.address, nil)
	}
	return ec.client.StorageByAddr(ctx.context, account.address, big.NewInt(number))
}

func (ec *EthereumClient) GetAccountInfo(ctx *Context, account *Address, number int64) (*types.StateAccount, error) {
	if number < 0 {
		return ec.client.AccountInfo(ctx.context, account.address, nil)
	}
	return ec.client.AccountInfo(ctx.context, account.address, big.NewInt(number))
}

func (ec *EthereumClient) GetStorageByAddrKeyAndNumber(ctx *Context, account *Address, blockNumber int64, startKey common.Hash, number int) (allStorage []map[string]common.Hash, _ error) {
	if number < 0 {
		return ec.client.StorageByAddrKeyAndNumber(ctx.context, account.address, nil, startKey, number)
	}
	return ec.client.StorageByAddrKeyAndNumber(ctx.context, account.address, big.NewInt(blockNumber), startKey, number)
}

func (ec *EthereumClient) BlockAccountInfo222(ctx *Context, blockNumber int64) (map[common.Hash]common.AccInfo, error) {
	return ec.client.BlockAccountInfo222(ctx.context, big.NewInt(blockNumber))
}

func (ec *EthereumClient) GetCodeText(ctx *Context, account *Address, number int64) (code string, _ error) {
	if number < 0 {
		return ec.client.CodeText(ctx.context, account.address, nil)
	}
	return ec.client.CodeText(ctx.context, account.address, big.NewInt(number))
}
