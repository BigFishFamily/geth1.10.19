...
func (t *Trie) TryGet222(key []byte) ([]byte, error) {
	value, newroot, didResolve, err := t.tryGet(t.root, keybytesToHex(key), 0)
	if err == nil && didResolve {
		t.root = newroot
	}

	fmt.Println("INFO [Trie->TryGet222, keybytesToHex(Bytes2Hex):", common.Bytes2Hex(keybytesToHex(key)), ", keybytesToHex(BytesToHash): ", common.BytesToHash(keybytesToHex(key)), "]")
	return value, err
}
