...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals, setHead bool) (int, error) {
	...
	sealHashChecked := make([]bool, len(chain))

	for i, block := range chain {
		headers[i] = block.Header()
		seals[i] = verifySeals
		sealHashChecked[i] = block.SealHashChecked
	}
	abort, results := bc.engine.VerifyHeadersSealHashChecked(bc, headers, seals,sealHashChecked)
	defer close(abort)
