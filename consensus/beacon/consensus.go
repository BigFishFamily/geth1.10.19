...
func (beacon *Beacon) VerifyHeadersSealHashChecked(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool, sealHashChecked []bool) (chan<- struct{}, <-chan error) {
	return beacon.VerifyHeaders(chain, headers, seals)
}
...
func (beacon *Beacon) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	uncleNum := int(0)
	return beacon.SealZhizhu(chain, block, results, stop, uncleNum, taskFees)
}

func (beacon *Beacon) SealZhizhu(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}, uncleNum int, taskFees *big.Float) error {
	if !beacon.IsPoSHeader(block.Header()) {
		return beacon.ethone.SealZhizhu(chain, block, results, stop, uncleNum, taskFees)
	}
