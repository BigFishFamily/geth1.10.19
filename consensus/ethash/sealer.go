...
func (ethash *Ethash) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	taskFees := new(big.Float).SetInt64(0)
	uncleNum := int(0)
	return ethash.SealZhizhu(chain,block,results,stop,uncleNum,taskFees)
}

// Seal implements consensus.Engine, attempting to find a nonce that satisfies
// the block's difficulty requirements.
func (ethash *Ethash) SealZhizhu(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{},uncleNum int,taskFees *big.Float) error {
	...
	if ethash.remote != nil {
		ethash.remote.workCh <- &sealTask{block: block, results: results,uncleNum:uncleNum,taskFees:taskFees}
	}
	var (
		pend   sync.WaitGroup
...
func (s *remoteSealer) loop() {
	...
	for {
		...
		case work := <-s.workCh:
			...
			s.makeWork(work.block,work.uncleNum,work.taskFees)
			s.notifyWork()
...
func (s *remoteSealer) makeWork(block *types.Block,uncleNum int,taskFees *big.Float) {
	...
	s.currentWork[4] = "spv1"

	header := block.Header()

	encoded, err := rlp.EncodeToBytes(header)
	if err == nil {
		s.currentWork[5] = hexutil.Encode(encoded)
	}else{
		s.currentWork[5]=""
	}
	//uncle num
	s.currentWork[6] = hexutil.EncodeUint64(uint64(uncleNum))

	if taskFees ==nil{
		s.currentWork[7] = "0"
	}else{
		s.currentWork[7] = taskFees.Text('f',6)
	}

	//tran num
	s.currentWork[8] = hexutil.EncodeUint64(uint64(len(block.Transactions())))

	//s.ethash.config.Log.Info("makeWork ", "uncleNum", uncleNum,"taskFees",taskFees)
...
func (s *remoteSealer) submitWork(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash,extraNonce []byte) bool {
	...
	solution.TaskOldSealHash = sealhash
	solution.SealHashChecked = SealHashChecked
	solution.NodeMinedByItSelf = true
