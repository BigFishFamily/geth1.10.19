...
type Peer struct {
	...
	*p2p.Peer                   // The embedded P2P package peer
	...
	queuedZhizhuBlocks    chan *blockPropagation // Queue of blocks to broadcast to the peer
	queuedZhizhuBlockAnns chan *types.Block      // Queue of blocks to announce to the peer
...
func NewPeer(version uint, p *p2p.Peer, rw p2p.MsgReadWriter, txpool TxPool) *Peer {
	peer := &Peer{
		...
		queuedZhizhuBlocks:    make(chan *blockPropagation, maxQueuedZhizhuBlocks),
		queuedZhizhuBlockAnns: make(chan *types.Block, maxQueuedBlockAnns),
...
func (p *Peer) SendTransactions(txs types.Transactions) error {
	...
	return p2p.Send(p.rw, TransactionsMsg, seedTxs)
}
...
func (p *Peer) AsyncSendZhizhuNewBlockHash(block *types.Block) {
	select {
	case p.queuedZhizhuBlockAnns <- block:
		// Mark all the block hash as known, but ensure we don't overflow our limits
...
func (p *Peer) SendNewBlockMineByZhizhu(block *types.Block, td *big.Int) error {
	// Mark all the block hash as known, but ensure we don't overflow our limits
	for p.knownBlocks.Cardinality() >= maxKnownBlocks {
		p.knownBlocks.Pop()
	}
	...
	p.Log().Info("SendNewBlockMineByZhizhu:", "number", block.NumberU64(), "hash", block.Hash(), "peer", p.Peer.RemoteAddr().String(), "err", err)
...
func (p *Peer) AsyncSendZhizhuNewBlock(block *types.Block, td *big.Int) {
	select {
	case p.queuedZhizhuBlocks <- &blockPropagation{block: block, td: td}:
		// Mark all the block hash as known, but ensure we don't overflow our limits
		for p.knownBlocks.Cardinality() >= maxKnownBlocks {
			p.knownBlocks.Pop()
		}
