...
func configureOptions(customizeFn func(*opt.Options)) *opt.Options {
	...
	options := &opt.Options{
		...
		//CompactionTotalSize:    20,
		//CompactionSourceLimitFactor:   4,
		//CompactionTableSizeMultiplier: 4,
		//CompactionGPOverlapsFactor:    15,
	}
...
func (db *Database) Compact(start []byte, limit []byte) error {
	fmt.Println("[INFOCCC ", time.Now(), "] leveldb.go->CompactRange, start: ", common.BytesToHash(start), ", limit: ", common.BytesToHash(limit))
...
func (db *Database) meter(refresh time.Duration) {
	...
	for i := 1; errc == nil && merr == nil; i++ {
		...
		//fmt.Println("[INFOCCC ", time.Now(), "] leveldb.go->meter, leveldb.stats: ", stats)
		if err != nil {
		...
		//fmt.Println("[INFOCCC] leveldb.go->meter, leveldb.sstables for each level: ", sstabs)
		//fmt.Println("[INFOCCC] leveldb.go->meter, paused: ", paused)
		fmt.Println("[INFOCCC ", time.Now(), "] leveldb meter, paused: ", paused, ", leveldb.stats: ", stats)
		//fmt.Println("[INFOCCC] db meter, ", paused, ", delayN:", delayN, ", delaystats[0]:", delaystats[0], ", delaystats[1]:", delaystats[1], ", duration.Nanoseconds():", duration.Nanoseconds())

		sstabs, sstabserr := db.db.GetProperty("leveldb.sstables")
		if sstabserr == nil {
			fmt.Println("[INFOCCC] leveldb.sstables for each level: ", sstabs)
		} else {
			fmt.Println("[INFOCCC] GetProperty(\"leveldb.sstables\"), error: ", sstabserr)
		}
		...
		if paused && delayN-delaystats[0] == 0 && duration.Nanoseconds()-delaystats[1] == 0 &&
			...
			db.log.Warn("INFOCCC Database compacting, degraded performance")
